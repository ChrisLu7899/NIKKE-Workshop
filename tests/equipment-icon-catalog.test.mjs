...
test("装备图标目录固定包含 12 件装备且每个部位各 3 件", () => {
  ...
  ["头部", "身躯", "臂部", "腿部"].forEach((slot) => {
    assert.equal(equipmentNamesBySlot(slot).length, 3);
  });
});

test("装备名称映射到固定部位与本地图标样本", () => {
  assert.equal(findEquipmentIconByName(" 99型头盔 ")?.slot, "头部");
  assert.equal(findEquipmentIconByName("Ｖ金属背心")?.slot, "身躯");
  assert.equal(findEquipmentIconByName("代码XXX手套")?.slot, "臂部");
  assert.equal(findEquipmentIconByName("代码xxx鞋")?.slot, "腿部");
