...
test("area-id fallback candidates stay scoped to the selected server family", () => {
  assert.deepEqual(getAreaIdProbeCandidates("global"), ["84", "82"]);
  assert.deepEqual(getAreaIdProbeCandidates("hmt"), ["91"]);
  assert.deepEqual(getAreaIdProbeCandidates("unknown"), []);

  const candidates = getAreaIdProbeCandidates("global");
  candidates.push("999");
  assert.deepEqual(getAreaIdProbeCandidates("global"), ["84", "82"]);
});
