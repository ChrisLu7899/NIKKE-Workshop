...
import { buildCharactersSignature } from "../../../utils/templateSignatures.js";
import {
  COMMON_CHARACTER_TEMPLATE_KEY,
  COMMON_CHARACTER_TEMPLATE_NAME,
  isCommonCharacterTemplate,
  upsertCommonCharacterTemplate,
} from "../../../data/commonCharacterList.js";
 ...
 * @param {boolean} options.enableAccountTemplates - 是否启用账号模板管理
 */
export function useTemplateManagement({
  ...
  const applyTemplatesWithDefault = useCallback(async (list, preferredId) => {
    ...
    if (!nextList.some(isCommonCharacterTemplate)) {
      const targetId = nextList.find((tpl) => tpl?.isDefault)?.id || nextList[0]?.id;
      nextList = nextList.map((tpl) => (tpl.id === targetId ? {
        ...tpl,
        name: COMMON_CHARACTER_TEMPLATE_NAME,
        isDefault: true,
        isFixed: true,
  ...
  const ensureCommonTemplate = useCallback(async (nikkeList) => {
    if (!Array.isArray(nikkeList) || !nikkeList.length) return null;
    const stored = await getTemplates();
    const hasTarget = (stored || []).some(isCommonCharacterTemplate)
      || (stored || []).some((template) => template?.isDefault)
      || Boolean(stored?.length);
    const newTemplateId = hasTarget ? "" : await getNextTemplateId();
    const result = upsertCommonCharacterTemplate({
      templates: stored || [],
      nikkeList,
      newTemplateId,
    });
    await setStoredTemplates(result.templates);
    templatesRef.current = result.templates;
    setTemplates(result.templates);
    const currentId = await getCurrentTemplateId();
    const nextSelectedId = result.templates.some((template) => template.id === currentId)
      ? currentId
      : result.template.id;
    setSelectedTemplateId(nextSelectedId);
    await setCurrentTemplateId(nextSelectedId);
    if (nextSelectedId === result.template.id) {
      setCharactersData(result.template.data);
      await persistCharacters(result.template.data);
    }
    return result;
  }, [setCharactersData]);
  ...
  const syncCharacterTemplateData = useCallback(async (templateId, data) => {
    ...
    const updated = {
      ...tpl,
      name: isCommonCharacterTemplate(tpl) ? COMMON_CHARACTER_TEMPLATE_NAME : tpl.name,
      data: data || {},
    };
  ...
  const handleCreateTemplateFromData = useCallback(async ({ data, name } = {}) => {
    const id = await getNextTemplateId();
    const template = {
      id,
      name: String(name || generateNextDefaultName()).trim() || generateNextDefaultName(),
      data: data || buildEmptyCharactersData(),
      createdAt: Date.now(),
    };
    await saveTemplate(template);
    ...
    setSelectedTemplateId(id);
    await setCurrentTemplateId(id);
    setCharactersData(template.data);
    await persistCharacters(template.data);
    showMessage(t("templateSaved"), "success");
    return id;
  }, [buildEmptyCharactersData, generateNextDefaultName, setCharactersData, showMessage, t]);

  const handleUpdateTemplateData = useCallback(async (id, data, { activate = true } = {}) => {
    const template = templatesRef.current.find((item) => item.id === id);
    if (!template || !data) return false;
    if (isCommonCharacterTemplate(template)) {
      showMessage("“常用”是固定列表，不能增删妮姬", "warning");
      return false;
    }
    const updated = { ...template, data };
    await saveTemplate(updated);
    setTemplates((previous) => previous.map((item) => (item.id === id ? updated : item)));
    if (activate) {
      setSelectedTemplateId(id);
      await setCurrentTemplateId(id);
      setCharactersData(data);
      await persistCharacters(data);
    }
    return true;
  }, [setCharactersData, showMessage]);
  ...
  const handleDeleteTemplate = useCallback(async (id) => {
    ...
    if (id === defaultTemplateId || isCommonCharacterTemplate(target)) {
  ...
  }, [defaultTemplateId, refreshTemplates, selectedTemplateId, showMessage, t, templates]);

  // 重命名模板
  const startRenameTemplate = useCallback((id) => {
    const template = templates.find((item) => item.id === id);
    if (isCommonCharacterTemplate(template)) {
      showMessage("“常用”是固定列表，不能重命名", "warning");
      return;
    }
    setIsRenaming(true);
    setRenameId(id);
  ...
  const confirmRename = useCallback(async () => {
    ...
    const tpl = templates.find(item => item.id === id);
    if (!tpl) return;
    if (isCommonCharacterTemplate(tpl)) {
      showMessage("“常用”是固定列表，不能重命名", "warning");
      return;
    }
  ...
  const handleTemplateChange = useCallback(async (id) => {
    ...
    await setCurrentTemplateId(id);
    if (id) {
      const tpl = templatesRef.current.find(item => item.id === id);
      if (tpl) {
        await applyTemplate(tpl);
        showMessage(t("templateLoaded"), "success");
      }
    }
  }, [selectedTemplateId, characters, syncCharacterTemplateData, applyTemplate, showMessage, t]);
  ...
  useEffect(() => {
    if (!enableAccountTemplates) return;
  ...
  }, [applyAccountTemplatesWithDefault, enableAccountTemplates]);

  // 监听 accountTemplates storage 变化（来自 popup 的更新）
  useEffect(() => {
    if (!enableAccountTemplates) return undefined;
  ...
  }, [enableAccountTemplates, selectedAccountTemplateId, setAccounts]);
