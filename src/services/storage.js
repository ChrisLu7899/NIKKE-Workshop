...
import { normalizeStoredAccounts } from "../domain/account.js";
...
const CALCULATOR_DATA_KEY = "calculatorData"; // 洗词条计算器使用的去敏角色装备快照
...
export const getCalculatorData = () =>
  new Promise((res) =>
    chrome.storage.local.get(CALCULATOR_DATA_KEY, (r) => res(r[CALCULATOR_DATA_KEY] || null))
  );

// 保存洗词条计算器快照。该数据不包含 Cookie、邮箱、密码和 game_uid。
export const setCalculatorData = (snapshot) =>
  new Promise((res) =>
    chrome.storage.local.set({ [CALCULATOR_DATA_KEY]: snapshot }, () => res())
  );
