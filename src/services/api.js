...
/* ========== 载入基础账号数据模板 ========== */
export const loadBaseAccountDict = async (charactersOverride = null) => {
  ...
  // 从存储中获取角色数据（角色管理系统）
  const charactersData = charactersOverride || await getCharacters();
...
export class ApiRequestError extends Error {
  constructor(message, { status = 0, endpoint = "", retryAfterMs = 0 } = {}) {
    super(message);
