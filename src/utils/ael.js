...
export function isUnowned(details) {
  ...
  const explicitOwned = details?.is_owned ?? details?.isOwned ?? details?.owned;
  if (explicitOwned === true) return false;
  if (explicitOwned === false) return true;

  // grade=0 是图鉴占位数据与零星突破角色都会出现的合法默认值，不能单独
  // 作为“已拥有”的证据。旧逻辑使用 grade >= 0，因而把绝大多数图鉴条目
  // 都误判为已获得。
  const hasLB = (typeof limit_break === 'object' && limit_break && (
    (typeof limit_break.grade === 'number' && limit_break.grade > 0)
    || (typeof limit_break.core === 'number' && limit_break.core > 0)
